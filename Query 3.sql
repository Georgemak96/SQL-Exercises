...
INSERT INTO seats (line, number, room_id)
...
SELECT SUBSTRING(email, 5) AS emai_new FROM customers;
SELECT UPPER(last_name) AS last_name, LOWER(first_name) AS first_name FROM customers
...
WHERE film_id=5);
SELECT c.first_name,c.last_name, COUNT(b.customer_id) AS bookings FROM customers c
JOIN bookings b ON c.id=b.customer_id
GROUP BY c.first_name, c.last_name
ORDER BY bookings DESC
LIMIT 5;

SELECT f.name, r.name, COUNT(*) AS screenings FROM films f
JOIN screenings s ON f.id=s.film_id
...
LIMIT 1;
SELECT COUNT(customers) AS customers FROM
(SELECT COUNT(*) AS customers FROM bookings b
LEFT JOIN customers c ON b.customer_id=c.id
GROUP BY b.customer_id)a;
###OR####
SELECT COUNT(DISTINCT(customer_id)) AS no_customers FROM bookings;
CREATE VIEW customers_table AS SELECT COUNT(DISTINCT(customer_id)) AS no_customers FROM bookings;
DROP VIEW customers_table;
SELECT * FROM films UNION SELECT * FROM bookings;
ALTER TABLE customers
ADD COLUMN george2 VARCHAR(20);
SELECT * FROM customers;
ALTER TABLE customers
ALTER first_name SET DEFAULT 'sang'; 
DESCRIBE customers;
ALTER TABLE customers
ADD COLUMN george2 INT NOT NULL DEFAULT '2';


##### Exercises workearly####
SELECT SUM(CASE WHEN status="closed"
THEN 1 ELSE 0 END)/ COUNT(DISTINCT(account_id) AS percentage_closed
FROM account_status
